...
const processSierra = (data: string[][]): ProcessedData => {
  ...
  const getSpec = (symbol: string) => {
    const base = symbol.toUpperCase()
    // minis / micros
    if (base.startsWith('MES')) return { tickSize: 0.25, tickValue: 1.25 }
    if (base.startsWith('ES')) return { tickSize: 0.25, tickValue: 12.5 }
    if (base.startsWith('MCL') || base.startsWith('MCLE')) return { tickSize: 0.01, tickValue: 1 }
    if (base.startsWith('CL')) return { tickSize: 0.01, tickValue: 10 }
    // crude fallback
...
// MetaTrader 5 history report (Excel)
const processMt5 = (data: string[][]): ProcessedData => {
  if (!data.length) throw new Error('The MT5 file appears to be empty.')

  // Find the most likely header row: longest row before an empty gap
  let headerIndex = 0
  let maxLen = 0
  data.forEach((row, idx) => {
    const len = row.filter(c => (c ?? '').toString().trim() !== '').length
    if (len > maxLen) {
      maxLen = len
      headerIndex = idx
    }
  ...
  const headers = rawHeaders.map(cell => (cell ?? '').toString().trim()).filter(Boolean)

  const body = data.slice(headerIndex + 1)
  const processedData: string[][] = []
  for (const row of body) {
    const clean = row.map(cell => (cell ?? '').toString().trim())
    if (!clean.length) continue
    // break at summary/footer markers
    const first = (clean[0] || '').toLowerCase()
    if (first.includes('closed') || first.includes('summary') || first.includes('total')) break
    if (clean.every(c => c === '')) continue
    processedData.push(clean)
  }

  if (!headers.length || !processedData.length) {
    throw new Error('Unable to parse MT5 report (no headers or rows found).')
  }
...
export const platforms: PlatformConfig[] = [
  ...
  },
  {
    ...
    type: 'mt5',
    name: 'MetaTrader 5',
    description: 'Import MT5 History Report (XLSX)',
    category: 'Platform CSV Import',
    details: 'Import your MetaTrader 5 history report (.xlsx or .csv)',
    logo: {
      component: () => <div className="w-8 h-8 rounded-md border border-border/50 flex items-center justify-center text-xs font-semibold">MT5</div>
    },
    skipHeaderSelection: true,
    requiresAccountSelection: true,
  ...
  },
  {
    ...
    logo: {
      component: () => <div className="w-8 h-8 rounded-md border border-border/50 flex items-center justify-center text-xs font-semibold">SC</div>
    },
    skipHeaderSelection: true,
